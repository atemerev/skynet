#!/usr/bin/env sh
# -*- wisp -*-
# precompile dependencies with regular scheme
guile -L $(dirname $(dirname $(realpath "$0"))) -c '(import (language wisp spec) (fibers) (fibers channels))' || (
    echo please install guile fibers and wisp, for example via '`guix package -i guile guile-fibers guile-wisp`';
    exit 1)
...
# time taskset -c 0-5
exec guile -L $(dirname $(realpath "$0")) --language=wisp -x .w -e '(skynet)' -s "$0" "$@"
; !#
...
define : skynet-nothread level index
  cond
     {level >= levels}
         ;; apply + : iota 1000 ;; takes around 50 us
         . index
     else
         let sum
           : remaining children
